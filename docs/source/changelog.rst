...
* Introducing the flow hospital - a component of the node that manages flows that have errored and whether they should
  ...
  flow will be parked and retried on node restart. This is to allow the node operator a chance to rectify the situation
  so otherwise the node will have an incomplete view of the ledger.
...
* New ``killFlow`` RPC for killing stuck flows, using the ``StateMachineRunId``.
