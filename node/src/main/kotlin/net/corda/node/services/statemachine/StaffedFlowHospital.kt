    ...
    object DoctorRetry : Staff {
        override fun consult(flowFiber: FlowFiber, currentState: StateMachineState, newError: Throwable, history: MedicalHistory): Diagnosis {
            if (newError is FlowRetryException) {
                val flowLogic = flowFiber.snapshot().flowLogic
                val retryableFlow = (flowLogic as? RetryableFlow)
                        ?: throw IllegalArgumentException("Unable to retry a non-retryable flow: ${flowLogic::class.java}")
                val maxRetries = retryableFlow.maxRetries
                if (history.notDischargedForTheSameThingMoreThan(maxRetries, this)) return Diagnosis.DISCHARGE
            }
